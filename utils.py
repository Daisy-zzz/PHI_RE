...
class WordEmbeddingLoader(object):
    ...
    def load_embedding(self):
        ...
        special_emb = np.random.normal(vec_mean, vec_std, (5, self.word_dim))
...
class SemEvalDateset(Dataset):
    ...
    def __load_data(self):
        ...
        data = []
        labels = []
        fr = pd.read_pickle(path_data_file)
...
class SemEvalDataLoader(object):
    ...
    def get_train(self):
        return self.__get_data('train.pkl', shuffle=True)

    def get_dev(self):
        return self.__get_data('dev.pkl', shuffle=False)

    def get_test(self):
        return self.__get_data('test.pkl', shuffle=False)
